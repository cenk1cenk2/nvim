...
A presentation skill, not a workflow. The report's shape, its converged-state gate, and its prerequisite marking are owned by `status-report`; this skill decides only how many reports come out of one invocation.
...
`/output-status-report` with no argument emits **exactly one** report of the current state, then stands down. No mode engages, nothing persists, and the next turn returns to whatever register the conversation was already in.
...
If the picture is still moving, say so and give the terse update instead — `status-report` gates the full shape on a converged state, and that gate holds here.

## Toggle

On/off mechanics per `mode-toggle`.

- **On:** `/output-status-report on`, "keep reporting", "report at every milestone", "status report each milestone", "stay on status".
...
While on, mark the posture in each report per `mode-toggle` — `[status-report]`, or both names when another posture is driving.

## While the toggle is on

- **Report at milestones, not at turns.** The trigger is `status-report`'s converged-state rule. A watcher firing, one agent reporting, or a single check landing is a terse update.
...
**User says:** "/output-status-report" after four agents were dispatched across a migration
...
1. Acknowledge in one line: status-report posture on, scoped to this migration.
2. Stay quiet through dispatches and watcher wakes, answering in the conversation's register.
3. When wave one finishes and its verdicts settle, emit the full report marked `[status-report]`.
