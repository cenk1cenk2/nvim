...
function M.init_defaults(languages)
  for _, entry in ipairs(languages) do if not lvim.lang[entry] then lvim.lang[entry] = {formatters = {}, linters = {}, lsp = {}} end end
end

local function is_overridden(server)
  local overrides = lvim.lsp.override
  if type(overrides) == 'table' then if vim.tbl_contains(overrides, server) then return true end end
end
...
local function resolve_config(name, user_config)
  local config = {on_attach = require('lvim.lsp').common_on_attach, on_init = require('lvim.lsp').common_on_init, capabilities = require('lvim.lsp').common_capabilities()}
...
function M.setup(server_name, user_config)
  ...
  if lsp_utils.is_client_active(server_name) or is_overridden(server_name) then return end
