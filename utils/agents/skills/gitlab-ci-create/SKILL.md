...
description: Create or update GitLab CI/CD pipelines using the devops/pipelines task-based model. Use when user says "add a pipeline", "set up GitLab CI", or "modify .gitlab-ci.yml". Do NOT use for diagnosing failures (gitlab-ci-failed), GitHub Actions (github-ci-create), or MR descriptions (gitlab-pr-create).
interaction: chat
...
- **`gitlab-ci-failed`** (resource: `skills://skill/gitlab-ci-failed`) — for diagnosing failures in existing GitLab CI pipelines. Auto-invoke when the user reports CI failures instead of wanting to create/update pipelines.
