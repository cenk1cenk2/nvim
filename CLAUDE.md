...
- **Sidekick CLI terminals use central Hyprpilot spawn**
  ...
  - Sidekick's `hyprpilot` tool should set `EDITOR = "nvim"`, `VISUAL = "nvim"`, and `NVIM_FLATTEN_DISABLE = "1"`; `flatten.nvim`'s `pipe_path` hook returns nil for that env so provider editor shortcuts can launch nested Neovim without flattening into the host.
  - Keep the Sidekick `hyprpilot spawn` command shape inline in the tool config and pass simple Sidekick target opts directly in mappings. Only `<Leader>ctt` should start a Sidekick terminal; context/focus mappings should target `filter = { attached = true }` so they do not spawn or switch sessions. Do not add one-use constants/helpers for the `hyprpilot-nvim` MCP server name, package name, config patch, or static session opts.
