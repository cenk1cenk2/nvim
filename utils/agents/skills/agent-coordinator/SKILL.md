...
5b. **⛔ Reap what you spawned.** A router accumulates agents and watchers faster than a worker does, so this is coordinator-specific debt: every stale entry makes the map you are holding less accurate, until you cannot tell what is genuinely in flight. Stop each one as soon as it stops earning its keep — delivered and acted on, answered another way, idle and taken back in-house, superseded, **or about to be replaced (reap BEFORE re-dispatching)**. Completion does not self-clean: finished agents and exited background tasks linger in the runtime's task list. Re-dispatching over the same target without reaping risks two concurrent writers silently clobbering each other. Before reporting a phase done, enumerate what you spawned and confirm each is stopped or *deliberately* still running with a stated reason.
