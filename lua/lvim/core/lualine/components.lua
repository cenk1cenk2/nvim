...
return {
  ...
  diagnostics = {'diagnostics', sources = {'nvim_lsp'}, symbols = {error = ' ', warn = ' ', info = ' ', hint = ' '}, color = {}, cond = conditions.hide_in_width},
  treesitter = {
    function()
  ...
  lsp = {
    function(msg)
      ...
      -- add client
      for _, client in pairs(buf_clients) do if client.name ~= 'null-ls' then table.insert(buf_client_names, client.name) end end
  ...
  spaces = {
    function()
      local label = 'Spaces: '
      if not vim.api.nvim_buf_get_option(0, 'expandtab') then label = 'Tab size: ' end
