...
function M.list_available(filetype)
  ...
  for _, provider in pairs(null_ls.builtins.formatting) do
    ...
    end) then table.insert(formatters, provider.name) end
  end
  ...
  for _, fmt_config in ipairs(formatter_configs) do
    ...
    if not formatter then
      ...
      if not formatter_cmd then
        ...
        formatters[fmt_config.exe] = formatter.with {command = formatter_cmd, extra_args = fmt_config.args, filetypes = fmt_config.filetypes}
      end
    end
  end
