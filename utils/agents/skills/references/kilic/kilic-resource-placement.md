...
- **A chart change ships on a version.** Charts release by semantic-release and are pinned per environment via `targetRevision` in `<env>/<component>/patch-applicationset.yaml`. Kargo (`cluster/kargo-root`) writes that pin, the first one included, and promotes it through environments. Editing `chart-*/values.yaml` changes nothing until the chart is released and the pin moves.
