...
One item is a sentence, never a one-row table; a batch armed together is one short table with a linked row per item. Handles, pids, watched paths and polling cadence stay in the internal ledger the owning skills define (`agent-watchers`, `hyprpilot-delegate`), quotable on request — never in the prose.
