...
local function to_32_bits_number(str)
  return bit.lshift(string.byte(str, 1), 24) + bit.lshift(string.byte(str, 2), 16) + bit.lshift(string.byte(str, 3), 8) + string.byte(str, 4)
end
...
local function sha1(val)
  ...
  local padded_message = val .. string.char(128) .. string.rep(string.char(0), 64 - ((string.len(val) + 1 + 8) % 64) + 4) .. to_32_bits_str(string.len(val) * 8)
...
local function decode_frame()
  ...
  while true do
    ...
    for i = current_byte, payload_end do
      result.payload = result.payload .. string.char(bit.bxor(string.byte(frame, i), string.byte(result.masking_key, j)))
      j = (j % 4) + 1
    end
...
function write_stdout(id, data)
  ...
  local lenstr =
    string.char(bit.band(len, 255), bit.band(bit.rshift(len, 8), 255), bit.band(bit.rshift(len, 16), 255), bit.band(bit.rshift(len, 24), 255), string.byte(data, 1, -1))
...
log = io.open(home_dir .. "/.local/state/nvim/surfingkeys.log", "a")
log:setvbuf("no")
...
function _G.surfingkeys_scratch(fn, content, type)
  local filetype = "markdown"
  local bufnr = vim.api.nvim_create_buf(false, false)
  local filename = string.format("%s/%s.%s", surfingkeys_tempdir, uuid(), filetype)

  vim.api.nvim_buf_set_name(bufnr, filename)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, vim.split(base64_dec(content), "\n"))
...
--         surfingkeys_notify("WriteData", vim.json.encode(vim.api.nvim_buf_get_lines(event.buf, 0, -1, false)))
...
function! NewScratch(fn, content, type)
    call v:lua.surfingkeys_scratch(a:fn, a:content, a:type)
endfunction
...
print("Surfing keys server ready.")
