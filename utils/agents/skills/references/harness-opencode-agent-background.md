...
| Work you dispatched yourself | depends on the build | If a background-subagent mechanism is present it notifies the parent on completion — do not poll it. See `harness-opencode-agents-delegate`. |
...
- **A dispatched `task` blocks the turn** (see `harness-opencode-agents-delegate`), so "arm it and keep working" is not the same posture as on Claude Code.
