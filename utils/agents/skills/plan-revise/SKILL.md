...
Same as `plan-hard`: before asking the user, check if the codebase answers the question. Use `mcp-diagnostics`, `git` MCP, `Grep`, `Glob`, `Read`, `github__*` / `gitlab__*`, and `context7` for documentation. Only escalate to the user for intent, preference, or unknowable-from-code decisions.
...
- **`code-assistant`** (`skills://skill/code-assistant`) — if the user is mid-implementation and the code-assistant is tracking progress, update its TodoWrite state to match the revised plan.
