...
styles.none = {
  style = 'none',
  options = {icons_enabled = true, component_separators = {left = '', right = ''}, section_separators = {left = '', right = ''}, disabled_filetypes = {}},
...
function M.get_style(style)
  ...
  if not vim.tbl_contains(style_keys, style) then
    ...
    Log:error('Invalid lualine style', string.format('"%s"', style), 'options are: ', string.format('"%s"', table.concat(style_keys, '", "')))
...
function M.update()
  ...
  if lvim.builtin.lualine.options.theme == nil then lvim.builtin.lualine.options.theme = lvim.colorscheme end
