...
- **Reply to a thread:** add a note to the discussion via `gitlab__create_merge_request_discussion_note` (`discussion_id` + `body`).
- **Resolve a thread:** use `gitlab__resolve_merge_request_thread` with `discussion_id` and `resolved: true`.
