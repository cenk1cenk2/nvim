...
function M.enable_profile()
  ...
  M.print_profile = function()
    M.profile["lvim.impatient"] = { resolve = 0, load = impatient_dur, loader = "standard" }
...
local function load_from_cache(name)
  ...
  if M.profile then
    M.profile[name] = { resolve = load_start - resolve_start, load = hrtime() - load_start, loader = "cache" }
  end
