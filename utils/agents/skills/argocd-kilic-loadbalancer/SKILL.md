...
description: argocd-kilic-loadbalancer Create or extend routing workloads in a load balancer cluster's ArgoCD repository. Scaffolds Pulumi services for cross-cluster routing, VM routing, or direct LB routes. Always manually invoked. Do NOT use for non-LB ArgoCD workloads (/argocd-kilic-workload), standalone workload repos (/cluster-kilic-workload), or Helm chart wrappers (/cluster-kilic-chart).
