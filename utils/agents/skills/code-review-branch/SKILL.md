...
description: Review the current branch's changes against the default or target branch with full conversation context. Always manually invoked. Do NOT use for PR/MR descriptions (github-pr-create, gitlab-pr-create), debugging (code-debug), or failed commands (code-task-failed).
interaction: chat
...
- **`gitlab-pr-create`** (resource: `skills://skill/gitlab-pr-create`) — for drafting MR descriptions after review. Do not auto-invoke.
