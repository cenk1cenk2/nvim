...
| `stderr` | `stderr.log` | the actual failure text behind a non-zero exit — report this, not a bare code |
...
- **Paging is MCP-style.** `cursor` in, `nextCursor` out, opaque. **No `nextCursor` means finished AND fully read.**
