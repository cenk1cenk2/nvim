...
function M.remove_template_files()
  ...
  for _, file in ipairs(vim.fn.glob(ftplugin_dir .. '/*.lua', 1, 1)) do vim.fn.delete(file) end
end
...
function M.is_ignored(server_name, filetypes)
  ...
  local blacklist = {'jedi_language_server', 'pylsp', 'sqlls', 'sqls', 'angularls', 'ansiblels'}
...
function M.generate_ftplugin(server_name, dir)
  ...
  if M.is_ignored(server_name, filetypes) then return end

  Log:debug('got associated filetypes: ' .. vim.inspect(filetypes) .. dir .. server_name)

  for _, filetype in ipairs(filetypes) do
    local filename = join_paths(dir, filetype .. '.lua')
    local setup_cmd = string.format([[require("lvim.lsp.manager").setup(%q)]], server_name)
    Log:debug('using setup_cmd: ' .. setup_cmd)
...
function M.generate_templates(servers_names)
  ...
  if vim.tbl_isempty(servers_names) then
    ...
    for _, server in pairs(available_servers) do table.insert(servers_names, server.name) end
  end

  -- create the directory if it didn't exist
  if not utils.is_directory(lvim.lsp.templates_dir) then vim.fn.mkdir(ftplugin_dir, 'p') end

  for _, server in ipairs(servers_names) do M.generate_ftplugin(server, ftplugin_dir) end
