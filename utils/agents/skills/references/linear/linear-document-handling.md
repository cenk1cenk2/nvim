...
Scope: a Linear document attaches to one project, initiative, issue, cycle, or team (`list_documents` / `get_document` / `save_document`). An issue reaches its own documents, its parent project's, and its attachments (`list_comments`, attachment links). "Attached document" below means any of these, plus linked plan-like files the conversation points at.
