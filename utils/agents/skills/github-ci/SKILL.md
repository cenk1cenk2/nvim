---
name: github-ci
description: Create or update GitHub Actions workflows for the current repository. Use when the user wants to add CI/CD pipelines, modify existing workflows, or set up new GitHub Actions.
interaction: chat
...
- Use `gh` CLI as fallback when MCP tools lack the needed capability.
...
3. **Research reference implementations.** If the user provides a reference repository, use `mcp__mcphub__github__get_file_contents` to read its workflows and understand how they are structured. Adapt the patterns to the current repository's conventions.
4. **Search for existing actions.** Use `mcp__mcphub__github__search_repositories` and `mcp__mcphub__github__search_code` to find well-maintained actions for the job. Evaluate candidates by: stars, recent activity, maintenance status, and community adoption. Prefer official actions (`actions/*`) and verified creators.
5. **Fetch latest versions.** For every action to be used, fetch its latest release via `mcp__mcphub__github__get_latest_release` or `mcp__mcphub__github__list_tags` from the action's repository. Use the latest stable version tag, never `@master` or `@main`.
6. **Consult documentation.** Read the action's `README.md` via `mcp__mcphub__github__get_file_contents` to understand all available inputs, outputs, and configuration options. Do not guess at configuration — verify from the source.
