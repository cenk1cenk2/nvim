...
M.setup = function()
  ...
  for _, exec in pairs(lvim.builtin.terminal.execs) do require('lvim.core.terminal').add_exec(exec[1], exec[2], exec[3]) end
  terminal.setup(lvim.builtin.terminal)

  if lvim.builtin.terminal.on_config_done then lvim.builtin.terminal.on_config_done(terminal) end
end

M.add_exec = function(exec, keymap, name)
  vim.api.nvim_set_keymap('n', '<leader>' .. keymap, '<cmd>lua require(\'lvim.core.terminal\')._exec_toggle(\'' .. exec .. '\')<CR>', {noremap = true, silent = true})
