---
name: gitlab-pr-read
description: Read and internalize the full state of a GitLab MR — description, comments, discussion threads, and diff. Use when user says "read this MR", "load the MR", "what's in this MR", or "summarize this MR". Purely informational — does not modify anything. Do NOT use for reviewing MRs (gitlab-pr-review), fixing MR threads (gitlab-pr-fix), or writing MR descriptions (gitlab-pr-create).
interaction: chat
