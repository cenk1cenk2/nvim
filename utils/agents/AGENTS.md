...
- **⛔ Loading a skill means loading its references.** `read_skill` returns the body **only** — declared references are a separate fetch. So every `read_skill` is followed by `load_skill_references { slug }` for that skill, in the same step, before acting on it. A skill body names its references inline (`per \`output-diff\``) and does not tell you to fetch them; this rule is why. A skill whose `references:` array is empty needs no second call.
