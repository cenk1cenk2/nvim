...
- **A skill's declared references arrive with it.** `read_skill` appends them by default, under a `skill_references:` banner with one `reference:` block per file. That is why a skill body names its references inline (`per \`output-diff\``) instead of telling you to fetch them. Pass `references: false` only when reading a skill in order to **edit** it; `load_skill_references { slug }` fetches the bundle on its own when you want it without the body.
...
Before creating or modifying resources outside the local workspace (GitHub/GitLab, Linear, Slack, Obsidian, Notion, etc.), summarize the intended change and wait for explicit approval unless the user has already given autopilot/proceed authorization for that class of write. If a catalog skill covers the write, route through it per §II "skill-first" — it carries the required fields and the approval gate. Guidance-file and repo-note updates follow §VII Knowledge Base Updates.
