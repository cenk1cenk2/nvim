...
Invoking this skill IS a standing blessing to spawn the PM and to talk to it. **Talking costs nothing and gates nothing.** What gates is Linear itself: tracker writes are presented before they land, unless the user has given a standing preapproval, in which case the PM applies and reports.

State that spans turns must be written durably per `long-running-work` — the PM's name and spawn id, its scope, its tier, the armed watchers, and the open ledger do not survive a compaction on their own.
...
- **Touches an MR or PR** — description, title, threads, merge state. That is `git-companion`'s record, and the description's magic-word trailers move Linear state on merge, so two companions editing it is the silent-clobber case in `agent-companion`.
- **Writes outside its scope.** Findings on a neighbouring project or a parent above the scope are reported, never edited.

## Watchers: the lead arms, the PM is told

**A PM who does not learn that something happened reconciles the tracker against a stale narrative** — the first failure mode a tracker layer exists to prevent. The companion cannot observe Linear or the platform changing, and it must not arm loops of its own, because a wake fired inside a detached subagent never reaches you.

1. **You arm every watcher**, through the `agent-background` skill, with the discipline in `agent-watchers` — one per independent condition, armed the moment it opens.
2. **A wake is a report to the PM.** Re-verify authoritatively on the main loop first, then send one message saying what actually changed.
3. **The PM tells you what to arm next.** An issue in flight whose MR or pipeline has no watcher is a gap, and the PM should say so.
...
2. **Pick the tier and spawn**, per `agent-companion`. Linear-specific signals when the user named no tier:
   ...
   | Signal | Tier |
   |---|---|
   | Flat scope, under ~10 issues, no cross-issue sequencing | `cheap`, and say why |
...
3. **Write the brief** with the contract in `agent-companion`, filled in for Linear:
   ...
   - **The re-read rule** from `agent-companion`: it re-reads the issues before asserting any state, and labels every claim observed or reported. A tracker answer from memory is the one thing that makes a PM worse than no PM.
...
The PM only knows what you tell it.
...
- One scope, one PM, alive for the whole section — the accumulated reasoning is the product, and the tracker state is always re-read.
- The split is one question: you already hold the finished text and the target, or you do not. When unsure, hand it over.
- Talking to the PM is free. Writing to Linear gates.
...
- It is the tracker layer and nothing else; implementation goes to `agent-coordinator` or a normal delegate, and MRs belong to `git-companion`.
- Every open condition gets a watcher you armed, and every wake becomes a message to the PM.
