...
- **Sidekick CLI terminals use central Hyprpilot spawn**
  ...
  - Sidekick's `hyprpilot` tool should set `EDITOR = "vim"` and `VISUAL = "vim"` so provider editor shortcuts (for example Claude Code `<C-g>`) do not launch nested Neovim through `flatten.nvim`.
