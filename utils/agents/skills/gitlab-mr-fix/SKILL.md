---
name: gitlab-mr-fix
description: Fix all open review conversations on a GitLab MR by reading each thread, understanding the requested change, and applying fixes to the code. Use when user says "fix the MR comments", "resolve the review", "address MR feedback", or "fix review threads". Do NOT use for reviewing MRs (gitlab-mr-review), writing MR descriptions (gitlab-mr), or GitHub PR fixes (github-pr-fix).
interaction: chat
