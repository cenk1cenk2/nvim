...
function M.setup()
  ...
  if vim.fn.has 'nvim-0.5.1' > 0 then
    vim.lsp.handlers['textDocument/publishDiagnostics'] = function(_, result, ctx, _)
      ...
      if ok then
        ...
        local sign_names = {'DiagnosticSignError', 'DiagnosticSignWarn', 'DiagnosticSignInfo', 'DiagnosticSignHint'}
        for i, sign in ipairs(lvim.lsp.diagnostics.signs.values) do vim.fn.sign_define(sign_names[i], {texthl = sign_names[i], text = sign.text, numhl = ''}) end
  ...
  vim.lsp.handlers['textDocument/signatureHelp'] = vim.lsp.with(vim.lsp.handlers.signature_help, {border = lvim.lsp.popup_border})
end
...
function M.show_line_diagnostics()
  ...
  local severity_highlight = {'LspDiagnosticsFloatingError', 'LspDiagnosticsFloatingWarning', 'LspDiagnosticsFloatingInformation', 'LspDiagnosticsFloatingHint'}
  local ok, vim_diag = pcall(require, 'vim.diagnostic')
  if ok then
    local buf_id = vim.api.nvim_win_get_buf(0)
    local win_id = vim.api.nvim_get_current_win()
    local cursor_position = vim.api.nvim_win_get_cursor(win_id)
    severity_highlight = {'DiagnosticFloatingError', 'DiagnosticFloatingWarn', 'DiagnosticFloatingInfo', 'DiagnosticFloatingHint'}
  ...
  for i, diagnostic in ipairs(diagnostics) do
    ...
    if source then
      if string.find(source, '/') then source = string.sub(diagnostic.source, string.find(diagnostic.source, '([%w-_]+)$')) end
      diag_message = string.format('%d. %s: %s', i, source, diag_message)
    else
      diag_message = string.format('%d. %s', i, diag_message)
    end
    if diagnostic.code then diag_message = string.format('%s [%s]', diag_message, diagnostic.code) end
