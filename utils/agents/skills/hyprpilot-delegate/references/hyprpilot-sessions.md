...
**Nor is it proof the ending is the one you are waiting for.** A steer seals the turn it interrupts, so its marker lands exactly like a finished turn's. The disk tells the two apart: a numbered directory under `turns/` above the one being watched means the session already started a newer turn, so this marker is an interruption arriving behind it. `hyprpilot-harness.py wait` makes that check on every marker it sees and exits **12** rather than 0 — which is why an unreaped watcher on a steered turn is a wake to discard rather than a wrong answer to collect. `143` in its `exitCode` is the SIGTERM the steer sent; `0` there means the turn finished before the steer landed and its own result is real.
...
- **A steer ends the interrupted turn**, so its `done.json` lands and its watcher fires on an interruption rather than an answer. That turn's directory survives and stays readable, but what the agent had done is only what the vendor persisted before the kill. Arm the new turn's watcher first and reap the interrupted one after: the reap is cleanup, because a watcher still on the sealed turn exits 12 rather than reporting an answer.
