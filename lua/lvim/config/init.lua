...
local function deprecation_notice()
  ...
  if in_headless then return end
  ...
  for lang, entry in pairs(lvim.lang) do
    ...
    if not vim.tbl_isempty(deprecated_config) then
      local msg = string.format('Deprecation notice: [lvim.lang.%s.lsp] setting is no longer supported. See https://github.com/LunarVim/LunarVim#breaking-changes', lang)
