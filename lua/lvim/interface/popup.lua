...
function Popup:new(opts)
  ...
  local popup_layout = {relative = 'editor', height = math.floor(editor_layout.height * 0.9), width = math.floor(editor_layout.width * 0.8), style = 'minimal', border = 'rounded'}
  ...
  local obj = {buffer = vim.api.nvim_create_buf(false, true), layout = vim.tbl_deep_extend('force', popup_layout, opts.layout), win_opts = opts.win_opts, buf_opts = opts.buf_opts}
...
function Popup:display(content_provider)
  ...
  -- window options
  for key, value in pairs(self.win_opts) do vim.api.nvim_win_set_option(self.win_id, key, value) end

  -- buffer options
  for key, value in pairs(self.buf_opts) do vim.api.nvim_buf_set_option(self.buffer, key, value) end
end
