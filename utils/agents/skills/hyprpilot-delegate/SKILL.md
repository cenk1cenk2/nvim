...
6. **Collect the result deliberately — this is where the work gets lost.**
   ...
   - **⛔ Default to skipping the paging entirely — this is the cheapest path and the one to reach for first.** `sessionInfo.files.transcript` is the path to `turns.jsonl`; `jq` the answer straight out rather than pulling 60 kB of tool traffic through your context. The `hyprpilot-sessions` reference carries a one-liner per vendor plus the full cost ranking.
