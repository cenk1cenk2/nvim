...
local function tbl_set_highlight(terms, highlight_group)
  for _, v in pairs(terms) do vim.cmd('let m=matchadd("' .. highlight_group .. '", "' .. v .. '[ ,│\']")') end
end
...
local function make_client_info(client)
  ...
  local client_info = {fmt('* Name:                 %s', name), fmt('* Id:                   %s', tostring(id)), fmt('* Supports formatting:  %s', tostring(document_formatting))}
...
function M.toggle_popup(ft)
  ...
  local header = {fmt('Detected filetype:      %s', ft), fmt('Treesitter active:      %s', tostring(next(vim.treesitter.highlighter.active) ~= nil))}
  ...
  local null_ls_info = {'Formatters and linters', fmt('* Configured providers: %s%s', table.concat(registered_providers, '  , '), registered_count > 0 and '  ' or '')}
  ...
  local Popup = require('lvim.interface.popup'):new{win_opts = {number = false}, buf_opts = {modifiable = false, filetype = 'lspinfo'}}
