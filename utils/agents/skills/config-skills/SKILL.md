...
| `linear-state-transitions.md` | Auto-advance triggers (pickup → In Progress, MR/PR open → In Review, merge → Done), never-downgrade guard, id extraction, silent-with-report contract. | Agent-dispatch skills (delegate, anonymous, team, sequential), PR/MR-create skills (gitlab-mr-create, github-pr-create), linear-issue-comment. |
