---
name: gitlab-pr-comment
description: Post a companion skill's output as a comment on the current GitLab MR. Use when user says "comment on the MR", "post this to the MR", or invokes alongside another skill to comment its output on GitLab. Do NOT use for MR descriptions (gitlab-pr-create), GitHub PR comments (github-pr-comment), or issue comments (use GitLab MCP directly).
interaction: chat
