...
ft.find_lua_ftplugins = function(filetype)
  ...
  local result = {}
  for _, pat in ipairs(patterns) do vim.list_extend(result, vim.api.nvim_get_runtime_file(pat, true)) end
...
ft.do_filetype = function(filetype)
  ...
  for _, file in ipairs(ftplugins) do
    ...
    if not f then
      ...
      if not ok then vim.api.nvim_err_writeln('Error while processing file: ' .. file .. '\n' .. msg) end
    end
  end
end
