...
3. **Present, then spawn.** Show the resolved profile, the working directory, and the prompt. `spawn` runs the profile's `command` as this user, so it is never a silent action — present it and wait for approval. On approval call `spawn { profile, prompt|file, cwd?, mode?, wait?, timeout_seconds?, args?, with_config? }`.
   ...
   - **A `cwd` that does not exist fails as a missing executable**, for every vendor: `spawning claude: No such file or directory`. The message is about the directory, not the binary — `which claude` proves nothing, and a re-spawn with the same `cwd` fails identically. Create the directory, or pass one that exists, before re-spawning. The launcher's internal ordering behind the message is **unverified**; the signature is what identifies it.
...
7. **Steer across turns with `session_send`, not `spawn`.**
   ...
   - **A steer ENDS the interrupted turn, so its `done.json` lands and its armed watcher fires.** That wake is an interruption, not an answer — and what keeps it from misleading you is the waiter's own check, never a kill that beats the marker. The steer sequence, in this order:
...
**codex read-only is `mode: "read-only"`, never `mode: "plan"`.** Its sandbox vocabulary is `read-only`, `workspace-write` and `danger-full-access` (`codex --sandbox`); `plan` is not one of them, so passing it selects nothing and the agent runs at whatever the profile already had. **Both flags ship together, and that pair is untested.** The `codex` agent in `~/.config/hyprpilot/config.yaml` passes `--approve-for-me`, documented as routing approvals *"through automatic review using the workspace-write sandbox"*. hyprpilot does **not** treat that as a mode override — `has_codex_mode_override` checks `--ask-for-approval`, `--sandbox` and `--dangerously-bypass-approvals-and-sandbox` only — so a `mode` you set is still projected and codex receives `--approve-for-me` alongside `--sandbox read-only`. Which one wins, or whether codex errors, is **unverified**; confirm before relying on a read-only codex delegate. Whether a sandbox mode drops tools from the registry or gates them at call time is likewise unmeasured, so the rule above — ask what the agent *can call*, not what it can see — still applies. As of codex-cli 0.153.4, hyprpilot `src/spawn/providers/codex.rs`.

**claude read-only is a prompt constraint plus an `args` tool block, never `mode: "plan"`.** Plan mode gates claude's tools through an allowlist hyprpilot injects, and a spawned claude agent's own `AGENTS.md` requires `hyprpilot-skills__list_skills` and `read_skill` on turn one — an allowlist that gates that discovery leaves the agent unable to load the conventions it was briefed to follow. So say read-only in the prompt and block the write tools through `args` (`--disallowedTools`, in claude's own kebab spelling of the injected servers); both land where they cannot reach the discovery. `mode: "plan"` stays correct on opencode.
