...
local function r_inspect_settings(structure, limit, separator)
  ...
  if ts == 'table' then
    for k, v in pairs(structure) do
      ...
      if tostring(k):match '[^%a_]' then k = '["' .. tostring(k) .. '"]' end
...
function utils.toggle_autoformat()
  if lvim.format_on_save then
    require('core.autocmds').define_augroups {autoformat = {{'BufWritePre', '*', ':silent lua vim.lsp.buf.formatting_sync()'}}}
...
function utils.reload_lv_config()
  ...
  vim.cmd('source ' .. utils.join_paths(get_config_dir(), 'lua', 'plugins.lua'))
...
function utils.gsub_args(args)
  if args == nil or type(args) ~= 'table' then return args end
  local buffer_filepath = vim.fn.fnameescape(vim.api.nvim_buf_get_name(0))
  for i = 1, #args do args[i] = string.gsub(args[i], '${FILEPATH}', buffer_filepath) end
  return args
end
...
function utils.file_contains(file, query)
  ...
  if not vim.tbl_isempty(stderr) then error(vim.inspect(stderr)) end
  if not vim.tbl_isempty(stdout) then error(vim.inspect(stdout)) end
  return false
end
