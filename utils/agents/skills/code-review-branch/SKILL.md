...
description: Review the current branch's changes against the default or target branch with full conversation context. Always manually invoked. Do NOT use for PR/MR descriptions (/github-pr, /gitlab-pr), debugging (/code-debug), or failed commands (/code-task-failed).
interaction: chat
...
- **`/code-deviations`** (`~/.config/nvim/utils/agents/skills/code-deviations/SKILL.md`) — when review reveals consistency deviations that are intentional user choices, apply the code-deviations handling pattern. Do not auto-invoke.
