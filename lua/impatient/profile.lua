...
function M.print_profile(profile)
  ...
  for module, p in pairs(profile) do
    ...
    if plugin then
      if not plugins[plugin] then plugins[plugin] = {module = plugin, resolve = 0, load = 0, total = 0} end
    ...
    if #module > name_pad then name_pad = #module end
  ...
  do
    local plugins_a = {}
    for _, v in pairs(plugins) do plugins_a[#plugins_a + 1] = v end
    plugins = plugins_a
  end
  ...
  add('%s┬───────────┬────────────┬────────────┬────────────┤', l)
  for _, p in ipairs(plugins) do add('%-' .. name_pad .. 's │ %9s │ %8.4fms │ %8.4fms │ %8.4fms │', p.module, p.loader, p.resolve, p.load, p.total) end
  add('%s┴───────────┴────────────┴────────────┴────────────┤', l)
  add('%-' .. name_pad .. 's                                                    │', 'By Module')
  add('%s┬───────────┬────────────┬────────────┬────────────┤', l)
  for _, p in pairs(modules) do add('%-' .. name_pad .. 's │ %9s │ %8.4fms │ %8.4fms │ %8.4fms │', p.module, p.loader, p.resolve, p.load, p.total) end
  add('%s┴───────────┴────────────┴────────────┴────────────┘', l)
