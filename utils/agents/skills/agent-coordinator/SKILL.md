...
3. **Dispatch with the return contract.** Prompts are self-contained (agents lack your conversation) but point at skills and tools — subagents in this harness are **aware** targets per `agent-target-capability`. Disjoint file scopes; worktrees for parallel writers per `agent-worktrees`.
