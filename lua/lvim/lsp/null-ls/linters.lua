...
function M.list_available(filetype)
  ...
  for _, provider in pairs(null_ls.builtins.diagnostics) do
    ...
    end) then table.insert(linters, provider.name) end
  end
  ...
  for _, lnt_config in pairs(linter_configs) do
    ...
    if not linter then
      ...
      if not linter_cmd then
        ...
        linters[lnt_config.exe] = linter.with {command = linter_cmd, extra_args = lnt_config.args, filetypes = lnt_config.filetypes}
      end
    end
  end
