...
1. Call `linear_kilic-dev__get_user` with `query: "me"` to identify the current user.
...
4. Call `linear_kilic-dev__list_issue_labels` to fetch **all available labels** for the workspace.
...
- **Linear MCP:** `linear_kilic-dev` — ALWAYS use `linear_kilic-dev__*` tools unless prompted otherwise.
