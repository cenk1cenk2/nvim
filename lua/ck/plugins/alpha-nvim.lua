...
function M.config()
  require("ck.setup").define_plugin(M.name, true, {
    ...
    on_setup = function(_, config)
      ...
      require("alpha").setup({
        layout = {
          ...
          -- {
          --   type = "text",
          --   val = { ("%s %s#%s"):format(nvim.ui.icons.git.Branch, require("ck.version").get_config_branch(), require("ck.version").get_config_sha()) },
          --   opts = {
          --     redraw = false,
          --     position = "center",
          --     hl = "DashboardFooter",
          ...
          -- {
          --   type = "text",
          --   val = { ("%s %s"):format(nvim.ui.icons.ui.Gear, require("ck.version").get_nvim_version()) },
          --   opts = {
          --     redraw = false,
          --     position = "center",
          --     hl = "DashboardFooter",
    ...
    autocmds = function()
      return {
        require("ck.modules.autocmds").set_view_buffer({ "alpha" }),

        require("ck.modules.autocmds").q_close_autocmd({ "alpha" }),
      }
    end,
  })
end
